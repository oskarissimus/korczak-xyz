...
One worker for every app — one worker **file**, `dist/sw.js`, serving every scope on the
origin.

It was written here as "iOS shares a single registration and one CacheStorage across Safari and
every home screen icon", and **that part is not safe to rely on.** Web push proved it: a phone with
Event Watch and Metro Watch both on its home screen registered a *separate* push subscription for
each — two live endpoints, each attributed by iOS to the app that created it — which cannot happen
if the two share one registration, a subscription being a property of one. Treat installed apps as
separate containers that happen to run the same worker source. Nothing about the precache tiers
changes: the tiers were always named by the page rather than discovered by the worker, so a
per-app cache is the shape they already had. What does change is anything that assumed state
reached through the registration is one copy — `events.md` has the case that was.
