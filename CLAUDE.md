...
| File | Covers |
|---|---|
| `.claude/rules/accounts.md` | Who may use the site: sign-up, the approval every account waits for, the panel at `/apps/admin/`, the two rules files that enforce it |
...
| `.claude/rules/shopping.md` | The shopping list: minted ids and the name match, `done` as a field, the chips, the one household grant |
...
| `.claude/rules/audio-guide.md` | The audio guide: the map and its pins, the Go backend in `audio-guide-function/` and its cutover, what a tap costs, the iOS audio unlock, the narration language |
