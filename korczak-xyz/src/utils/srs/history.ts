/*
 * Aggregates over the review log: what one sitting came to, and what the whole history looks like.
 *
 * All of it is derived from the log and the deck, so nothing here is stored except the daily
 * mastery snapshots — and those exist only because "how many cards were mature last Tuesday" is
 * the one question the log cannot answer cheaply. Everything else is recomputed, which means a
 * scheduler change is reflected in the stats instead of contradicted by them.
 *
 * Card ids are opaque here, exactly as in `queue.ts`. Where a summary wants to split by the kind
 * of question asked, the caller passes a `directionOf` that knows how to read its own ids.
 */

import type { Bucket, Card, MasterySnapshot, ReviewEvent, SessionRecord } from './types';
import { bucketOf } from './scheduler';
...
export interface DirectionSplit {
  /** Which kind of question, as the caller's `directionOf` named it. */
  direction: string;
  answers: number;
  correct: number;
  totalMs: number;
...
/**
 * @param directionOf how to read the kind of question out of a card id. An id this build cannot
 *   parse returns null and is left out of the split rather than filed under a made-up heading.
 */
export function summarizeSession(
  events: ReviewEvent[],
  directionOf: (cardId: string) => string | null = () => null
): SessionSummary {
  const times = events.map((e) => e.ms);
  const totalMs = times.reduce((sum, ms) => sum + ms, 0);
  const correct = events.filter((e) => e.correct).length;

  const splits = new Map<string, DirectionSplit>();
  const misses = new Map<string, number>();
  for (const event of events) {
    const direction = directionOf(event.cardId);
    if (direction) {
      const split =
        splits.get(direction) ?? { direction, answers: 0, correct: 0, totalMs: 0 };
      split.answers++;
      split.correct += event.correct ? 1 : 0;
      split.totalMs += event.ms;
      splits.set(direction, split);
    }
