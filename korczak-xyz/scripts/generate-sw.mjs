...
/*
 * The pure halves, inlined in order rather than imported: dist/sw.js stays a classic script, which
 * is the safest kind to hand to iOS.
 *
 * They share ONE top-level scope once concatenated, so a name declared in two of them is a
 * SyntaxError — and there is one worker for every installed app on this origin, so that takes the
 * songbook offline along with everything else. swBundle.test.js parses the concatenation to catch
 * it. Only a leading `export ` is stripped, so a surviving `import` is the same failure by another
 * route; that test checks for those too.
 */
const PURE = ['sentry.js', 'routing.js', 'push.js'];
const pure = (
  await Promise.all(
    PURE.map(async (name) => (await readFile(join(swDir, name), 'utf8')).replace(/^export /gm, '')),
  )
).join('\n');
const body = await readFile(join(swDir, 'sw.template.js'), 'utf8');

/*
 * The Sentry release, so a worker error and a page error from the same deploy read as one
 * release. BUILD_ID identifies the precache contents and changes whenever an asset hash does;
 * the commit is what a diff can be read against, so the worker reports both.
 */
let commitHash = 'dev';
try {
  commitHash = execSync('git rev-parse --short HEAD').toString().trim();
} catch {
  // No git here (tarball checkout); 'dev' is honest about that.
}

const output = `// Generated by scripts/generate-sw.mjs. Edit src/sw/ instead.
const BUILD_ID = ${JSON.stringify(buildId)};
const COMMIT_HASH = ${JSON.stringify(commitHash)};
